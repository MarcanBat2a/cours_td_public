...
3. ★ Expliquez pourquoi la fusion combine environ log₂ n niveaux avec
   un travail linéaire par niveau. Pour un million : prédisez le compteur
   avec le modèle empirique 0,9 · n · log₂ n, puis le temps. Mesurez et
   expliquez l’écart : le coefficient 0,9 est une approximation.
...
3. Repérez deux appels qui recalculent le même résultat dans l’arbre
   de `fib(4)`. Gardez ce constat pour le TD 8.
...
2. ★ Comparez le nombre d'appels en fonction de n. Pourquoi stocker
   ...
   coût des opérations sur de grands entiers.

## Étape 3 - La copie cachée
...
On introduit ici un tri rapide dont le pivot est le premier élément :
...
## Étape 1 - Fibonacci, avec mémoire

Au TD 5, les mêmes sous-problèmes étaient recalculés. On conserve
maintenant leurs résultats dans un dictionnaire.
...
def fib_memo(n):
    ...
    if n < 2:
        return n
    # Si n manque dans memo, calculer puis enregistrer fib(n).
    # Renvoyer le résultat mémorisé.
```

1. Complétez. Comparez le résultat à `fib(n)` pour n de 0 à 15.
2. Comptez les appels pour `fib_memo(30)`, puis `fib_memo(100)`.
   **Avant chaque mesure indépendante : `memo.clear()` et `appels = 0`.**
3. ★ Trouvez la formule du nombre d'appels pour `n >= 2` et la classe.
   Comparez à la croissance du TD 5. Que conserve-t-on en mémoire ?
4. Appelez une deuxième fois `fib_memo(30)` sans vider `memo`, mais en
   remettant `appels` à zéro. Pourquoi est-ce une autre expérience ?

## Étape 2 - Sommes d'intervalles
